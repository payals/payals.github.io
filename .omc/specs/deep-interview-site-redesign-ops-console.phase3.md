...
## Phone page (Payal, 2026-09-04, after viewing the live site on a phone)

"There's a lot in there, lots of text, it's confusing and overwhelming, and the topic chips seem to do nothing when I touch them." Diagnosis: the chips work but dimming to 0.76 is invisible on a phone and the matches sit below the fold; the console is a desktop object squeezed onto a phone. Decision: desktop untouched; under 768px the same URL renders a separate simple page; the folded console from MB1-MB15 remains reachable as an opt-in "full console" view.

### PP. Phone page (below 768px, at `/`)
- PP1. One document, two experiences: index.html gains a server-rendered `<section class="phone-page">` (Liquid over the same data) shown only below 768px, while the console, topbar telemetry, legend bar, slider, terminal and status bar are `display: none` there. On desktop the phone page is never rendered visibly. Duplicate ids are not allowed: phone sections use `m-` ids and a small script maps `#talks`, `#writing`, `#cv`, `#now`, `#links` to them on phones.
- PP2. Console JavaScript does not initialise on phones (boot log, charts, palette, zoom, scrubber, topstats, constellation, mobile folds) unless the URL carries `?console=1`, in which case the folded console from MB1-MB15 renders instead of the phone page. The phone page's only script is under 60 lines: hash mapping and nothing else.
- PP3. Content, in order, single column, body 16px system sans, mono only for small labels: name and positioning line; "now" as four short lines (role, next talk with date and venue linked, latest post linked, reading with the past-reads link); talks: the upcoming talk plus the three most recent, each one line of date and title with the venue on a second line, then a native `<details>` "all 12 talks" with the full list and evidence links; writing: the latest five posts (date, title, minutes) and a link to /blog/; cv: headline, two roles with dates, education line, "download pdf", and a native `<details>` with the full cv.md; links: the two-column grid; a last line "phone view · open the full console" linking to `/?console=1`.
- PP4. No charts, no legend, no slider, no terminal, no status bar, no boot log on the phone page. No text below 13px; every tap target at least 44px tall; headings sticky is not needed.
- PP5. Page height at 390x844 at most 1,650px with both details closed (raised from 1,600px on 2026-09-04 so the first-screen links can meet the 44px target floor); Lighthouse accessibility at or above 97 on mobile; zero console errors; the phone page adds at most 8KB gzipped to the landing.
- PP6. Blog pages are unchanged. The topbar on phones shows only the name link and the four nav links, pointing at phone sections.
- PP7. In the full console on phones (`?console=1`), a topic filter hides non-matching rows instead of dimming them and shows a pill "topic · N items · clear" above the panes, so a tap has a visible result; the same pill appears on desktop only when a filter is active (dimming stays on desktop).

## Minimal phone page, second pass (Payal, 2026-09-04: "looks worse now, we need a redesign for mobile or at least minimalistic principles, too much text")

The first phone page was a text dump: every row a link, no hierarchy, no whitespace, a wrapping nav row. Replace it with a card and three doors. Binding over PP3 and PP5.

### PM. Minimal phone landing (below 768px, `/`)
- PM1. Structure, top to bottom, single column, 24px side margins, generous vertical rhythm (at least 20px between blocks): (1) name at 28px semibold; (2) "Staff platform engineer" at 18px; (3) one muted 15px line "Postgres · data platforms · AI-native systems"; (4) a now block of at most two lines: the role ("Staff Data Engineer at Lore") and the next talk as the only link in the block ("Speaking at Postgres Summit US, Sep 30"); (5) three door cards, each at least 72px tall and full width, tappable as a whole: **Talks** with "12 talks · next Sep 30", **Writing** with "8 posts · latest Sep 1", **CV** with "13 years around Postgres"; (6) a single row of four plain links: github, linkedin, email, rss; (7) one muted footer line "open the full console" linking to `/?console=1`. Nothing else: no lists, no dates column, no handles, no reading line, no topbar nav (the name is the only thing in the header; the console topbar nav is hidden on phones).
- PM2. Doors are real pages: Talks opens `/talks/` (new Jekyll page, `page` layout, rendered from data/talks.json: upcoming first, then by date, each talk as title, venue, date, evidence links; archive leads in a short final section), Writing opens `/blog/`, CV opens `/cv/` (new Jekyll page rendered from data/cv.md via markdownify with a download-pdf link at the top and the remaining links, codeberg, medium, older, reading, at the bottom). Both new pages must use `.markdown` files with front matter and permalinks `/talks/` and `/cv/`; on desktop they simply exist as pages (the console keeps its panes).
- PM3. Type: system sans everywhere on the phone landing; mono only for the small muted labels if any; no underlines except on the next-talk link and the four link row; the door cards use a subtle border on `--surface`, no underline, the title in ink and the subtitle in ink-2.
- PM4. Height at 390x844 at most 1,000px; at most 9 links on the whole landing (next talk, three doors, four link row, console line); Lighthouse accessibility 100 mobile; zero console errors; the phone still fetches no console modules.
- PM5. All numbers on the cards are computed by Liquid from the data files (talk count, next talk date, post count, latest post date, years since 2013), never hand-typed.
- PM6. HTTP fallback: `_includes/head.html` gains a first inline script that, when `location.protocol === "http:"` and the host is payalsingh.me, does `location.replace` to the https URL. Cloudflare's "Always Use HTTPS" is the real fix and is a dashboard setting Payal owns.
